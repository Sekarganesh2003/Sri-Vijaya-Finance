import {ChangeDetectionStrategy, Component, signal, computed, afterNextRender, effect, PLATFORM_ID, inject, untracked} from '@angular/core';
...
export class App {
  ...
  private readonly themeStorageKey = 'sri_finance_theme';
  readonly autoEndDateOffsetDays = 100;
  ...
  isDarkMode = signal<boolean>(this.readStoredDarkMode());
  ...
  settings = signal<AppSettings>({
    companyName: 'Sri Vijaya',
    englishName: 'Sri Vijaya',
  ...
  customerForm = new FormGroup({
    ...
    tenure: new FormControl<number>(100, { nonNullable: true }),
  ...
  settingsForm = new FormGroup({
    companyName: new FormControl('Sri Vijaya', { nonNullable: true, validators: [Validators.required] }),
    englishName: new FormControl('Sri Vijaya', { nonNullable: true, validators: [Validators.required] }),
    branch: new FormControl('Srivilliputhur', { nonNullable: true, validators: [Validators.required] }),
  ...
  constructor() {
    ...
    // Keep the selected theme and root classes in sync.
    effect(() => {
      ...
      const shouldRenderCharts = untracked(() => this.loginSuccess() && this.activeTab() === 'dashboard');
      if (shouldRenderCharts) {
        setTimeout(() => this.renderCharts(), 50);
      }
    });

    // Sync database state back to storage.
    effect(() => {
      this.saveDatabase();
    });

    // Render charts once the dashboard is visible and authenticated.
    effect(() => {
  ...
  initSupabaseConfig(): void {
    ...
    if (!savedBaseUrl && isMobileWebview) {
      // Default to the production server URL as the secure proxy API gateway
      savedBaseUrl = 'https://ais-pre-xax7y56dh5ad77myavgggs-973290280149.asia-southeast1.run.app';
      localStorage.setItem('supabase_api_base_url', savedBaseUrl);
      console.log(`[Mobile Native Mode] Pre-populated Supabase API base URL with: ${savedBaseUrl}`);
    } else if (!savedBaseUrl) {
      // For web, resolve dynamically by default (relative paths)
      savedBaseUrl = '';
    }

    this.supabaseApiBaseUrl.set(savedBaseUrl);
    this.addDiagnosticLog(`Supabase API Base URL initialized: "${savedBaseUrl || 'Relative Path (Web Default)'}"`);
  ...
  getApiUrl(path: string): string {
    const base = this.supabaseApiBaseUrl().trim();
    if (!base) return path;
  ...
  async loadFromSupabase(silent = false): Promise<void> {
    ...
    try {
      ...
      if (result && result.success && result.data && Object.keys(result.data).length > 0) {
        ...
        FinanceDB.saveAllData(exportObj).catch((e) => console.warn('Local indexedDB save failed:', e));
  ...
  onAddCustomer(): void {
    ...
    this.customerForm.reset({
      ...
      loanAmount: null,
      interestRate: null,
      tenure: this.autoEndDateOffsetDays,
      collectionGroup: '',
      clNo: ''
    });
  ...
  getClosingDate(): string {
    ...
    try {
      ...
      d.setDate(d.getDate() + this.autoEndDateOffsetDays);
      return getLocalDateString(d);
    } catch {
      return '';
    }
  ...
  onStartDateChange(value: string): void {
    ...
    this.customerForm.patchValue({ tenure: this.autoEndDateOffsetDays }, { emitEvent: false });
    this.saveDraft();
  }
